...
```bash
cd ns-3  # or your ns-3 tree (standalone flow)
./ns3 run "diffserv-l4s-fqcodel-comparison --mode=l4s-wred"
./ns3 run "diffserv-l4s-fqcodel-comparison --mode=l4s-coupled-only"
./ns3 run "diffserv-l4s-fqcodel-comparison --mode=l4s-fqcodel-classic"
./ns3 run "diffserv-l4s-fqcodel-comparison --mode=fqcodel"
```
...
```cpp
// Mode l4s-fqcodel-classic: mainline FqCoDelQueueDisc as the classic-side
...
- `--offeredBps` higher: p' climbs; ECDF tail extends right; all three L4S modes still show the same p' (the controller is rate-agnostic)
...
**To compare classic AQM variants**: run each `--mode` and diff the stdout summaries in `output/ns3/l4s-fqcodel/`; the `Disc marked` and `Disc dropped` lines show how mark/drop counts differ across WRED, coupled-only, FqCoDel-classic, and baseline FqCoDel — re-invoke `./scripts/plot-recipe l4s-coupled-marking` after each run.

### Try changing

1. Bump per-flow load to 8 Mbps (`--offeredBps=8000000`) — total 16 Mbps stresses all four discs harder; re-invoke `./scripts/plot-recipe l4s-coupled-marking` to see p' shift right as the controller responds.
...
The L4S client is address-family agnostic. `stratum::GetEcn` reads the `ECT(1)`
identifier from the IPv6 Traffic Class byte exactly as it does from the IPv4
header, so the DualPI2 coupled marking in this chapter's recipes runs unchanged
over IPv6. A minimal worked example, `l4s-ipv6-recipe`, runs an ECT(1) flow
against a classic Not-ECT flow on a 10 Mbps bottleneck:

```bash
./ns3 run "l4s-ipv6-recipe"
```

The L4S flow holds ~23 ms mean one-way delay against the classic flow's ~46 ms
(tail past 200 ms) — the same low-latency separation as over IPv4. Full capture
and the shared dual-stack mechanism:
