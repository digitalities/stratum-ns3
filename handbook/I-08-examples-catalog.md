...
Three ways to find what to run and how to configure it:

- **This catalogue** lists every example and what it does.
- **`./ns3 run "<example> --PrintHelp"`** prints the authoritative, always-current
  option list for any example — it is generated by ns-3 from the program itself,
  so it never goes out of date.
- **`--<flag>=list`** enumerates the valid values of catalogue-style options
  (for example `--aqm=list`, `--scheduler=list`).

[Appendix E — Command-line reference](appendix-E-cli-reference.md) collects the
`--PrintHelp` tables for the runnable examples in one place.

## The C++ API reference

The recipes show the helper API by example (`helper.AddTokenBucketPolicy(edge, {.codePt = 46, ...})`); the full class, method, and configuration-struct reference is generated from the source rather than hand-maintained here, so it never falls out of date:

- **Doxygen** documents every helper class and method, and every designated-initialiser configuration struct (`TokenBucketPolicySpec`, `CakeOptions`, `MarkRule`, ...), with a comment on each field. Generate it with `./ns3 docs doxygen` and open the HTML; the substrate's classes live under the `ns3::stratum` namespace.
- **`doc/stratum.rst`** is the curated model overview — the helper composers, the four-slot pipeline, and the three client surfaces.
- The client chapters in Part II ([II-05](II-05-diffserv-client.md), [II-06](II-06-l4s-client.md), [II-07](II-07-cake-client.md)) explain the same API conceptually.
...
### `diffserv-ipv6-recipe.cc`

**Purpose**: DiffServ EF / AF21 / BE per-class differentiation over a pure-IPv6 dumbbell. Three UDP CBR flows cross a 2 Mbps bottleneck; the DiffServ edge queue disc reads the DS field from the IPv6 Traffic Class byte and schedules with strict-priority queueing — EF and AF21 each receive their full offered load while BE absorbs what remains.

**Client**: DiffServ. **Recipe**: [appendix-D-ipv6-dual-stack.md → "DiffServ EF / AF21 / BE differentiation over IPv6"](appendix-D-ipv6-dual-stack.md).
...
**Purpose**: Compare Stratum L4S behaviour against a pure ns-3 mainline `FqCoDelQueueDisc` baseline on the same bottleneck scenario. Mode selectable via `--mode={l4s-wred, l4s-coupled-only, l4s-fqcodel-classic, fqcodel, classic-only}`.
...
**Purpose**: L4S DualPI2 coupled marking over a pure-IPv6 dumbbell. An ECT(1)-marked flow and a classic Not-ECT flow contend for a 10 Mbps bottleneck; the L4S sub-queue delivers low-latency coupled marking while the classic sub-queue operates with drop-based PI² AQM, with ECN codepoints read from the IPv6 Traffic Class byte.

**Client**: L4S. **Recipe**: [appendix-D-ipv6-dual-stack.md → "L4S ECT(1) coupled marking over IPv6"](appendix-D-ipv6-dual-stack.md).
...
### `cake-l4s-composition-ipv6.cc`

**Purpose**: CAKE diffserv4 + L4S composition over IPv6. A CAKE diffserv4 edge with a DualPI2 inner queue serves two TCP flows that share the Best-Effort tin: a scalable DCTCP flow that emits `ECT(1)` and lands in the L4S sub-queue (receiving CE marks under load), and a classic Cubic flow in the classic sub-queue. The `ECT(1)` codepoint is read from the IPv6 Traffic Class byte via the same extraction path as IPv4. Demonstrates that the CAKE + L4S composition is substrate-identical over IPv6.

**Client**: CAKE + L4S (advanced composition). **Recipe**: [appendix-D-ipv6-dual-stack.md → "CAKE + L4S composition over IPv6"](appendix-D-ipv6-dual-stack.md).
...
**Purpose**: CAKE diffserv4 tin classification over a pure-IPv6 dumbbell. Four UDP CBR flows carry distinct DSCP values — CS1 (Bulk), CS0 (BE), AF21 (Video), EF (Voice) — and CAKE's DSCP-to-tin classifier reads the DS field from the IPv6 Traffic Class byte to assign each flow to its correct tin and enforce the per-tin DRR shares.

**Client**: CAKE. **Recipe**: [appendix-D-ipv6-dual-stack.md → "CAKE diffserv4 tin classification over IPv6"](appendix-D-ipv6-dual-stack.md).
