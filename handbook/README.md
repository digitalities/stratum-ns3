...
| [appendix-D-ipv6-dual-stack.md](appendix-D-ipv6-dual-stack.md) | IPv6 dual-stack: the DS field occupies the same Traffic Class byte position as in IPv4, so client configuration is address-family agnostic; the shared networking delta stated once, plus worked DiffServ/L4S/CAKE examples over a pure-IPv6 dumbbell with captured output. |
| [appendix-E-cli-reference.md](appendix-E-cli-reference.md) | Command-line reference: the `--PrintHelp` option tables (flag / default / description) for the runnable examples, generated from the binaries. |
